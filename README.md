...
- [My linux configurations (or Dotfiles)](#my-linux-configurations-or-dotfiles)
    ...
    - [Enable hibernation](#enable-hibernation)
      - [Ubuntu 20.04 (with encrypted root partition and swap file)](#ubuntu-2004-with-encrypted-root-partition-and-swap-file)
    - [Disable Suspention/Hibernation Wake Up](#disable-suspentionhibernation-wake-up)
    - [Disable systemctl power options](#disable-systemctl-power-options)
    - [Disable suspend on lock screen](#disable-suspend-on-lock-screen)
    - [Turn builtin monitor on/off](#turn-builtin-monitor-onoff)
    - [Extension Swap Partition/File](#extension-swap-partitionfile)
    - [Screenkey/peek recorder](#screenkeypeek-recorder)
    - [ksuperkey](#ksuperkey)
    - [Disable auto update pop up](#disable-auto-update-pop-up)
    - [Disable pmproxy eating 100% CPU](#disable-pmproxy-eating-100-cpu)
    - [Restart audio interface](#restart-audio-interface)
...
1. **sync system plugins into custom `~/.local` build** (run after `build_xfce4.sh` or after installing new system plugins)
   ```bash
   # Copy all missing .desktop files and symlink their .so files into
   # the paths expected by the locally built panel
   mkdir -p ~/.local/share/xfce4/panel/plugins ~/.local/lib/xfce4/panel-plugins
   for desktop in /usr/share/xfce4/panel/plugins/*.desktop; do
       cp -v --update=none "$desktop" ~/.local/share/xfce4/panel/plugins/
       module=$(grep 'X-XFCE-Module=' "$desktop" | cut -d= -f2)
       so="/usr/lib/x86_64-linux-gnu/xfce4/panel/plugins/lib${module}.so"
